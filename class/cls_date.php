...
class ngs_date {
    ...
    public function x_week_start($date) {
        $ts = strtotime($date);
        $start = (date('w', $ts) == 4) ? $ts : strtotime('last thursday', $ts);
        return date('Y-m-d', $start);
    }

    public function x_week_s($date) {
        $ts = strtotime($date);
        $start = strtotime('last thursday', $ts);
        return date('Y-m-d', $start);
    }

    public function x_week_e($date) {
        $ts = strtotime($date);
        $start = strtotime('next thursday', $ts);
        return date('Y-m-d', $start);
    }

    public function convert24Hours($time_am_pm) {
        return date('H:i', strtotime($time_am_pm));
    }

    public function convertAMPM($time_24) {
        return date("h:i A", strtotime($time_24));
    }

    function calculateApproximateTime($roster_time, $actual_time) {
        $adjusted_time = '';
        $time_resolution = 15;
        $time_roster = round(strtotime($roster_time) / 60, 0);
        $time_actual = round(strtotime($actual_time) / 60, 0);
        $time_diff = $this->getDuration_mn((date('Y-m-d') . ' ' . $actual_time), (date('Y-m-d') . ' ' . $roster_time . ':00'));
        if ($time_diff == 0) {
            $adjusted_time = strtotime($roster_time) / 60; // if roster time is same as actual time
        }

        if ($time_diff == 5 || $time_diff < 5) {
            $adjusted_time = strtotime($roster_time) / 60; // if actual time is higher or equal to 5 minutes
        }

        if ($time_diff > 5 || $time_diff < 15) {
            $adjusted_time = strtotime($roster_time) / 60 + 0.25; // if actual time is lesser or equal to 15 minutes
        }

        if ($time_diff > 15 || $time_diff == 15) { // if actual time is eqal or higher than to 15 minutes
            $multiplier_in = round($time_diff / $time_resolution);

            if ($multiplier_in == 1) {
                $adjusted_time = strtotime($roster_time) / 60 + 0.25;
            } else {
                $timeadjust = ($multiplier_in * 0.25);

                $adjusted_time = (strtotime($roster_time) / 60) + ($timeadjust * 60);
            }
        }

        $adjusted_time_insecond = $adjusted_time * 60;
        return date("h:i A", $adjusted_time_insecond);
    }
