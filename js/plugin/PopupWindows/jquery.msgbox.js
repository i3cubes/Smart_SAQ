...
//            console.log(sidebarwidth)
            ...
            var DEFAULTS = {
                ...
                zIndex: 100, // the initial z-index of overlay and wrap. Just in case some other elements in document have greater z-index
...
//		console.log(options.id)
            ...
            MSGBOX.prototype = {
                ...
                _completeOptions: function () {
                    ...
                    if (($.inArray(this.options.type, ['ajx']) > -1) && this.options.url != "") {
                        this.options.content = this.options.url;
//                        console.log(this.options.url + " "); /* edit */
                    }
...
//                                 console.log("NOT URL");
                    ...
                    if (this.options.photoFade === true)
                        this.options.photoFade = DEFAULTS.photoFade;
                    if (this.options.photoFade === false)
                        this.options.photoFade = 0;
                ...
                _assemble: function () {
                    ...
                    // title
                   //if(this.$title){}
//                    console.log(this.options.modalType); /* edit */
                    
                    if (this.options.modalType == "false"){
                ...
                _append: function () {
                    ...
                    if (this.options.buttons !== null && this.options.buttons.length > 0) {
                        ...
                        if (this.options.resize)
                            this.$resize.appendTo(this.$foot);
                    } else {
                        if (this.options.resize)
                            this.$resize.appendTo(this.$content);
                    }
                },
                _bindEvents: function () {
                    ...
                    // the trigger
                    var closepopup = this.options.closeConfirm;
                    if (this.$trigger) {
                    ...
                    if (this.options.overlay && this.options.overlayEvent) {
                        ...
                        if (fn)
                            this.$overlay.bind('click.' + this.options.prefix, function () {
                                fn.apply(that);
                            });
                    }
                    ...
                    $.each(this.options.buttonEvents, function (button, fn) {
                        if (!$.isFunction(fn))
                            fn = $.isFunction(that[fn]) ? that[fn] : false;
                        if (!fn)
                            return;
...
//                                                    alert(closepopup);
                ...
                _loadImg: function ($handler, callback) {
                    ...
                    if (!this.titleSpecified)
                        this.title($handler.attr('title'));
                    ...
                    var imgload = function () {
                        ...
                        if (that.options.onLoad)
                            that.options.onLoad.apply(that);
                        if (callback)
                            callback.apply(that);
                    };
...
//                                    console.log("jdnfjdsn"); /* edit */
                        ...
                        case 'iframe':
                            ...
                            this.$iframe = createElement('iframe', this.options.prefix + '-iframe', {
                                ...
                                if (!that.options.title) {
                                    ...
                                    } // not available for cross-domain
                                    that.title(title);
                                }
                                that.loaded = true;
                                if (that.options.onLoad)
                ...
                enableDrag: function () {
                    ...
                    this.$title.css('cursor', 'move').bind('mousedown.drag.' + this.options.prefix, function (e) {
                        ...
                        $(document).one('mouseup.drag.' + that.options.prefix, function (e) {
                            ...
                            if ($.isEmptyObject(that.dragData))
                                return;
                            ...
                            if ($container.length > 0) {
                                ...
                                if (!offset)
                                    offset = {left: 0, top: 0}; // if it is window
                                ...
                                //console.log("Maleft "+maleft + "Mleft "+mleft + " Left "+left + " Top "+ top);
                            }
                ...
                enableResize: function () {
                    ...
                    this.$resize.show().bind('mousedown.resize.' + this.options.prefix, function (e) {
                        ...
                        $(document).bind('mouseup.resize.' + that.options.prefix, function (e) {
                            ...
                            if ($.isEmptyObject(that.resizeData))
                                return;
                ...
                play: function (callback) {
                    if (this.options.type != 'album' && this.options.type != 'gallery')
                        return;
                ...
                open: function (callback) {
                    ...
                    var rightAfterOpen = function () {
                        ...
                        if (that.options.onOpen)
                            that.options.onOpen.apply(that);
                        if (callback)
                            callback.apply(that);
                    };
                ...
                close: function (callback) {
                    ...
                    if (this.options.onBeforeClose && this.options.onBeforeClose.apply(this) === false)
                        return;
                        ...
                        that.animate({width: that.options.initialWidth, height: that.options.initialHeight, opacity: 0}, function () {
                            that.$overlay.fadeOut('fast', function () {
                                ...
                                if (that.options.onClose)
                                    that.options.onClose.apply(that);
                            });
                        });
//                                this.remove();
//                                that = this
//                           alert(that.options.id);  
                        $(document.body).removeData('msgbox.' + that.options.id);
//                           options['id']='';
                        that.remove();

                    };

                    if (!this.$trigger && this.minimized)
                ...
                max: function (callback) {
                    ...
                    if (this.minimized) {
                        ...
                        this.animate({
                            top: 0 + headerHeight,
                            left: 0 + sidebarwidth,
                            width: ($(window).width() - parseInt(sidebarwidth)),
                            height: winheight()
                        }, function () {
                            that.maximized = orgState;
                            that.$icons.max.addClass(that.options.prefix + '-restore');
...
//		console.log(options);
                ...
                if (typeof options == 'object') {
                    ...
                    options = options || 0;
                    var msgbox = $(this).data('msgbox.' + options.options.id, msgbox);
//			console.log(msgbox);
                    return msgbox;
                }
                return this;
